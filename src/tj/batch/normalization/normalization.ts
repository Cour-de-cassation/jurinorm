  ...
  while (decisionList.length > 0) {
    for (const decisionFilename of decisionList) {
      try {
        ...
        logger.info({
          ...normalizationFormatLogs,
          msg: `Check diff with previous version of decision ${decisionWithRules.sourceId} (if any)...`
        })
        ...
        if (previousVersion !== null) {
          ...
          if (diff.major && diff.major.length > 0) {
            ...
            if (
              ...
              logger.warn({
                ...normalizationFormatLogs,
                msg: `Decision has a bad updated date: ${decisionWithRules.dateDecision}`
              })
            } else if (
              ...
              logger.warn({
                ...normalizationFormatLogs,
                msg: `Decision has a codeDecision in blocked codeDecision list: ${decisionWithRules.endCaseCode}`
              })
