import {
    ...
    DEPRECATED_S3_BUCKET_NAME_RAW_TCOM
} from '../../library/env'
...
async function getMetadata(filename: string): Promise<CollectDto> {
...
async function createFromS3(
    ...
    const { metadonnees } = await getMetadata(filename)
...
async function main() {
    ...
    return Promise.all(files.map(createFromS3))
}
